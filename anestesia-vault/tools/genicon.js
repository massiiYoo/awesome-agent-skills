...
const A=hex('#c62828'),B=hex('#e53935'),WHITE=[255,255,255];
...
// protocol document
function doc(px,py){return inRR(px,py,0.31,0.25,0.69,0.75,0.05);}
// medical cross near the top of the document
function cross(px,py){
  const v=px>=0.473&&px<=0.527&&py>=0.315&&py<=0.455;
  const h=px>=0.43&&px<=0.57&&py>=0.357&&py<=0.413;
  return v||h;
}
// text lines below the cross
function lines(px,py){
  return inRR(px,py,0.37,0.55,0.63,0.583,0.016)
      || inRR(px,py,0.37,0.62,0.63,0.653,0.016)
      || inRR(px,py,0.37,0.69,0.55,0.723,0.016);
}
 ...
 for(let y=0;y<N;y++)for(let x=0;x<N;x++){let r=0,g=0,b=0,a=0;
  for(let sy=0;sy<s;sy++)for(let sx=0;sx<s;sx++){const ux=(x+(sx+0.5)/s)/N,uy=(y+(sy+0.5)/s)/N;let col=null,al=0;
    if(inRR(ux,uy,0,0,1,1,0.225)){col=lerp(A,B,(ux+uy)/2);al=255;
      if(doc(ux,uy))col=WHITE;
      if(cross(ux,uy)||lines(ux,uy))col=lerp(A,B,(ux+uy)/2);
    }
